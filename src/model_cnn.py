"""Small CNN tuned for small face datasets like Olivetti."""
import time
import os
import numpy as np
...
def train(X_train, y_train, X_val=None, y_val=None,
    ...
    n_classes = int(max(y_train.max(), y_val.max() if y_val is not None else 0)) + 1
    model = build_cnn(X_train.shape[1:], n_classes)

    cbs = [
        callbacks.EarlyStopping(
            monitor="val_accuracy" if X_val is not None else "accuracy",
            patience=25, mode="max", restore_best_weights=True,
        ),
        callbacks.ReduceLROnPlateau(
            monitor="val_loss" if X_val is not None else "loss",
            factor=0.5, patience=10, min_lr=1e-5, mode="min",
        ),
    ]
